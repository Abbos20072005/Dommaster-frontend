...
export const ReviewsList = () => {
  ...
  const getReviewsQuery = useInfiniteQuery({
    queryKey: ['reviews'],
    queryFn: ({ pageParam }) =>
      getReviews({
  ...
  const onLoadMore = () => {
    getReviewsQuery.fetchNextPage();
  };

  const reviews = getReviewsQuery.data?.pages.flatMap((page) => page.data.result.content);
  ...
  if (reviews?.length === 0) {
    return (
      <div className='flex h-full w-full flex-col items-center justify-center gap-4 py-20'>
        <p className='text-center font-bold'>{t('No reviews found')}</p>
      </div>
    );
  }
